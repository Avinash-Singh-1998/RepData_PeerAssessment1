...
was **`r format(meanStepsPerDay, digits = 2, nsmall = 2)`** while the **median**
was **`r medianStepsPerDay`**.
...
is the one that starts at **`r mostActiveTime`**, with an average of
...
We see that there are **`r rowsWithNA`** rows with missing values. In particular,
we have `r missingSteps` missing values on the steps variable, `r missingDate`
missing from date and `r missingInterval` missing from interval.
...
from **`r format(meanStepsPerDay, digits = 2, nsmall = 2)`** to
**`r format(meanStepsPerDayImputed, digits = 2, nsmall = 2)`** while the median
changed from **`r medianStepsPerDay`** to
**`r format(medianStepsPerDayImputed, digits = 2, nsmall = 2)`**.
